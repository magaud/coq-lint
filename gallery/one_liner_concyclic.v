...
Proof.
intros A B C D [HCop [O1]]; spliter ; destruct (col_dec A B C); [ exists O1; repeat split; Cop | destruct (l11_62_existence A B C O1) as [O []] ; exists O ; repeat split; try apply cong2_per2__cong with O1 O1; finish ].
Qed.
...
Proof.
intros ; split; [ unfold Concyclic in *; spliter; CopR | apply concyclic_aux in H0 ; apply concyclic_aux in H1 ; decompose [ ex and ] H0; clear H0 ; decompose [ ex and ] H1; clear H1 ; exists x ; repeat split; Cong ; assert (x = x0); [ assert_diffs ; apply is_circumcenter_uniqueness with A B C; try assumption; [ repeat split; [ CongR.. | Cop ] | repeat split; [ CongR.. | Cop ] ] | subst ; Cong ] ].
Qed.

Lemma concyclic_perm_1: forall A B C D,
  Concyclic A B C D -> Concyclic A B D C.
Proof.
intros A B C D H ; destruct H as [H1 [X H2]] ; split; [ Cop | spliter; exists X; repeat split; CongR.. ].
Qed.
...
Lemma concyclic_1123 : forall A B C,
 ...
 Concyclic A A B C.
Proof.
intros A B C HABC ; unfold Concyclic ; split; [ apply coplanar_trivial | destruct (exists_circumcenter A B C HABC) as [G HG] ; exists G ; apply circumcenter_cong in HG; spliter; repeat split; Cong ].
Qed.
