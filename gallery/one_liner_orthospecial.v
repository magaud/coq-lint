...
Proof.
intros A B C H [HNCol [HPerp]] ; apply coplanar_perm_6, perp__coplanar, HPerp.
Qed.
Ltac copR := CopR. 
...
Proof.
intros A B C X1 X2 X3 HNC HPar1 HPar2 HPar3 ; apply cop_npar__inter_exists; [ copR | intro HNPar; apply HNC ; assert (HFalsePar : Par B C A C) by
  (apply (par_trans B C B X1 A C); finish; apply (par_trans B C A X3 B);
    finish) ; perm_apply (par_id C B A) ].
Qed.

Lemma not_col_par_col_diff : forall A B C D E,
  ~ Col A B C -> Par A B C D -> Col C D E -> A <> E.
Proof.
intros A B C D E HNC HPar HC ; intro; subst ; apply HNC; apply not_strict_par1 with D E; finish.
Qed.
...
Lemma construct_triangle : forall A B C,
  ...
  D <> E /\ D <> F /\ E <> F.
Proof.
intros A B C HNC ; assert_diffs; rename H2 into HAB; rename H1 into HBC; rename
  H4 into HAC ; elim (parallel_existence1 A B C HAB); intros X1 HX1 ; elim (parallel_existence1 A C B HAC); intros X2 HX2 ; elim (parallel_existence1 B C A HBC); intros X3 HX3 ; assert (T : exists D, Col D B X2 /\ Col D C X1) by
  (apply construct_intersection with A X3; finish); 
  DecompExAnd T D ; assert (T : exists E, Col E A X3 /\ Col E C X1) by
  (apply construct_intersection with B X2; finish); 
  DecompExAnd T E ; assert (T : exists F, Col F A X3 /\ Col F B X2) by
  (apply construct_intersection with C X1; finish); 
  DecompExAnd T F ; assert (A <> E) by ((apply not_col_par_col_diff with B C X1; finish)) ; assert (A <> F) by ((apply not_col_par_col_diff with C B X2; finish)) ; assert (B <> D) by ((apply not_col_par_col_diff with A C X1; finish)) ; assert (B <> F) by ((apply not_col_par_col_diff with C A X3; finish)) ; assert (C <> D) by ((apply not_col_par_col_diff with A B X2; finish)) ; assert (C <> E) by ((apply not_col_par_col_diff with B A X3; finish)) ; assert (Par A B C D) by ((apply par_col_par with X1; finish)) ; assert (Par A C B D) by ((apply par_col_par with X2; finish)) ; assert (Par B C A E) by ((apply par_col_par with X3; finish)) ; assert (Par A B C E) by ((apply par_col_par with X1; finish)) ; assert (Par A C B F) by ((apply par_col_par with X2; finish)) ; assert (Par B C A F) by ((apply par_col_par with X3; finish)) ; assert (~ (D = E /\ D = F)); [ intro; spliter; treat_equalities ; assert_paras_perm ; assert_nparas_perm ; permutation_intro_in_hyps ; contradiction | exists D; exists E; exists F ; assert_diffs ; repeat split; try ColR; finish; [ intro; subst ; assert (E <> F) by ((intro; subst; intuition)) ; apply HNC; apply col_permutation_1; apply not_strict_par1 with E A;
  sfinish | intro; subst ; assert (E <> F) by ((intro; subst; intuition)) ; apply HNC; apply col_permutation_2; apply not_strict_par1 with E C;
  sfinish | intro; subst ; assert (D <> F) by ((intro; subst; intuition)) ; apply HNC; apply not_strict_par1 with D B; sfinish ] ].
Qed.
...
Proof.
intros A B C D E HD HNC HC HPar1 HPar2 HPar3 HPar4 ; assert (HPara1 : Parallelogram_strict A B C E) by
  (apply parallel_2_plg; finish) ; assert (HPara2 : Parallelogram_strict C A B D) by
  (apply parallel_2_plg; finish) ; assert_congs_perm ; apply cong_col_mid; Col; CongR.
Qed.
...
Lemma altitude_is_perp_bisect : forall A B C O A1 E F,
  ...
  Perp_bisect A O E F.
Proof with finish.
intros ; apply perp_mid_perp_bisect; finish ; apply perp_sym ; apply cop_par_perp__perp with B C; finish; [ apply par_col_par with A; finish | apply perp_col1 with A1; finish ].
Qed.
...
Lemma altitude_intersect:
 ...
 Col O C C1.
Proof with finish.
intros A A1 B B1 C C1 O HNC HPerp1 HPerp2 HPerp3 HC1 HC2 ; pose proof HNC as HT ; apply construct_triangle in HT ; destruct HT as [D [E [F HT]]] ; spliter ; assert (Midpoint A E F) by
  (apply diff_not_col_col_par4_mid with B C; finish) ; assert (Midpoint B D F) by
  (apply diff_not_col_col_par4_mid with A C; finish) ; assert (Midpoint C D E) by
  (apply diff_not_col_col_par4_mid with A B; finish) ; assert_diffs ; elim (eq_dec_points A O); intro; [ treat_equalities; apply col_permutation_4;
  apply cop_perp2__col with A B; finish ; apply perp_right_comm; apply perp_col1 with B1; finish | elim (eq_dec_points B O); intro; [ treat_equalities; apply col_permutation_4;
  apply cop_perp2__col with A B; finish ; apply perp_col1 with A1; finish | elim (eq_dec_points C O); intro; [ subst; Col | assert (Perp_bisect A O E F) by
  (apply altitude_is_perp_bisect with B C A1; finish) ; assert (Perp_bisect B O D F) by
  (apply altitude_is_perp_bisect with A C B1; finish) ; assert (Perp O C D E); [ apply circumcenter_intersect with F A B; finish; [ apply perp_bisect_sym_1; assumption | apply perp_bisect_sym_1; assumption ] | assert (Perp C1 C D E); [ apply perp_sym; apply cop_par_perp__perp with A B; finish ; apply par_symmetry; apply par_col_par_2 with C; finish | apply col_permutation_2; apply cop_perp2__col with D E; Perp ; apply coplanar_pseudo_trans with A B C; Cop ; apply coplanar_perm_2, col_cop__cop with B1; Col; Cop ] ] ] ] ].
...
Lemma is_orthocenter_cases :
  ...
  is_orthocenter G A B C.
Proof.
intros ; decompose [ or ] H; clear H; unfold is_orthocenter in *; spliter;
  repeat (split; finish).
Qed.
...
Proof.
intros ; unfold is_orthocenter in * ; spliter ; repeat split; finish.
Qed.

Lemma is_orthocenter_perm_1 : forall A B C G,
 is_orthocenter G A B C -> is_orthocenter G A C B.
Proof.
intros ; apply is_orthocenter_perm in H; intuition.
Qed.
...
Lemma orthocenter_per :
 ...
 H=B.
Proof.
intros ; unfold is_orthocenter in *; spliter ; assert_diffs ; assert (Perp A B B C) by ((apply per_perp; finish)) ; assert (Par A H A B) by ((apply l12_9 with B C; Cop)) ; assert (Col A B H) by (perm_apply (par_id A B H)) ; assert (Par C H B C) by ((apply l12_9 with A B; finish)) ; assert (Col B C H) by (perm_apply (par_id C B H)) ; apply l6_21 with A B C B; finish.
Qed.
...
Lemma orthocenter_col :
 ...
 H = B \/ H = C.
Proof.
intros ; unfold is_orthocenter in * ; spliter ; assert (Perp_at H B C A H); [ apply l8_14_2_1b_bis; finish | induction (eq_dec_points B H); [ subst; auto | assert (Perp A H B H) by ((apply (perp_col1 A H B C H); finish)) ; assert (Par A H A C) by ((apply l12_9 with B H; finish)) ; assert (Col H A C) by (perm_apply (par_id A C H)) ; assert (H = C); [ assert_diffs ; apply l6_21 with B C A C; finish | subst ; auto ] ] ].
Qed.

End Orthocenter.
#[global]
Hint Resolve is_orthocenter_perm_1 is_orthocenter_perm_2
  is_orthocenter_perm_3 is_orthocenter_perm_4 is_orthocenter_perm_5:
  Orthocenter.
#[global]Hint Resolve is_orthocenter_coplanar: cop.
