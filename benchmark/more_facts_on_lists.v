...
(* Equality type class and operations on lists which require a decidable (boolean) equality *) 
...
Proof.
  ...
  rewrite Bool.orb_false_iff in H0.
  split.
...
Proof.
  revert f; functional induction (distinct l) using distinct_ind.
  simpl; reflexivity.
  intros; simpl.
  assert (Hf:False).
  apply Bool.diff_true_false.
  ...
  intros.
  simpl.
  case_eq (belongs x (filter f xs)).
  intros.
  rewrite <- belongs_false_true with (f:=f) (x:=x) (xs:=xs); assumption.
...
Class Ordered_Type A `{Eq A}  :=
  {
    leb: A -> A -> bool;
    leb_flip: forall x y, leb x y = false -> leb y x = true;
    leb_eqb: forall x y, leb x y = true -> leb y x = true -> eqb x y = true;
    lemma: forall y a x, leb y a = true -> leb x a = false -> leb x y = true -> False;
  }.
...
Proof.
  intros y a x Hya Hxa H2.
  apply PeanoNat.Nat.leb_le in Hya; apply PeanoNat.Nat.leb_nle in Hxa;
    apply PeanoNat.Nat.leb_le in H2; lia.
Qed.

#[export] Instance Ordered_Nat : Ordered_Type nat :=
  {
    leb:=Nat.leb;
...
Proof.
  ...
  simpl; apply eq_refl.
  simpl; apply eq_refl.
  replace (sort (x::y::_x)) with (insert x (sort (y::_x))) by apply eq_refl.
  revert H1; rewrite and_bool; intros H1.
...
Proof.
  ...
  apply eq_refl.
  simpl; case_eq (leb x x0); intros.
  simpl.
  apply and_bool; split; solve [assumption|apply erefl].
  simpl.
  apply and_bool; split.
  apply leb_flip; assumption.
  apply eq_refl.

  simpl.
  case_eq (leb x x0); intros.
  simpl sorted.
  apply and_bool.
  split; assumption.
  ...
  simpl in Hs.
  rewrite H3 in Hs.
  change ((leb x0 y && sorted (y::insert x _x))=true).
  apply and_bool.
  split.
  assumption.
  assumption.
  simpl; rewrite H3; apply eq_refl.
Qed.
...
Proof.
  ...
  intros; simpl.
  case_eq (leb x y); intros Hle1.
  case_eq (leb y x); intros Hle2.
  assert (Heq:x=y) by (apply eqb_eq; apply leb_eqb; assumption).
  subst; apply eq_refl.  
  apply eq_refl.
  case_eq (leb y x); intros Hle2.
  apply eq_refl.
  specialize (leb_flip _ _ Hle1) as H1'.
  specialize (leb_flip _ _ Hle2) as H2'.
  assert (Heq:x=y) by  (apply eqb_eq; apply leb_eqb; assumption).
  subst; apply eq_refl.  
  intros.
  assert (Hs:(sorted l)=true).
  inversion H.
  destruct l.
  trivial.
  generalize (and_bool_lr _ _ H1); solve [intuition].
  simpl (insert y (a :: l)).
  case_eq (leb y a); intros Hya.
  simpl ((insert x (a :: l))).
  
  case_eq (leb x a); intros Hxa.
  simpl.
  rewrite Hya, Hxa.
  case_eq (leb x y); case_eq (leb y x); intros.
  assert (Hxy:x=y) by (apply eqb_eq; apply leb_eqb; assumption).
  rewrite Hxy; apply eq_refl.
  apply eq_refl.
  apply eq_refl.
  assert (Hxy:x=y) by (apply eqb_eq; apply leb_eqb; apply leb_flip in H2; apply leb_flip in H3; assumption).
  rewrite Hxy; apply eq_refl.

  simpl.
  rewrite Hya, Hxa.
  case_eq (leb x y); intros.
  assert (Hf:False) by eapply (lemma _ _ _ Hya Hxa H2).
  destruct Hf.
  apply eq_refl.
  ...
  simpl.
  rewrite Hya.
  case_eq (leb y x); intros.
  assert (Hf:False) by apply (lemma _ _ _ Hxa Hya H2).
  destruct Hf.
  apply eq_refl.
...
Proof.
  ...
  destruct x.
  assumption.
  apply and_bool_lr in H4; destruct H4; rewrite H4 in *; rewrite H3; tauto. 
